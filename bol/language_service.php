 ...
 * @since 1.0
 */
class LOCALIZATION_BOL_LanguageService
{
    const LANG_ID_VAR_NAME = "base_language_id";
    const CORE_LANG_PREFIX = 'core';
    /**
     * @var BOL_Language
     */
    ...
    {
        $this->languageDao = LOCALIZATION_DAO_Language::getInstance();
    ...
    public function getIsoCode($tag = null) {
        if(!$tag) {
            $language = $this->getCurrent();
            $tag = $language->getTag();
        }
        preg_match("/^([a-zA-Z]{2})$|^([a-zA-Z]{2})-([a-zA-Z]{2})(-\w*)?$/", $tag, $matches);
        $result = array();
        if(!empty($matches)) {
            if(!empty($matches[1])) {
                $result['code'] = strtolower($matches[1]);
                $result['country'] = strtoupper($matches[1]);
            } elseif(!empty($matches[2])) {
                $result['code'] = strtolower($matches[2]);
                $result['country'] = strtoupper($matches[3]);
            }
        }
        return $result;
    }
    ...
    public function importPrefix( $xml, $refreshCache=false, $importOnlyActivePluginPrefix = false, $updateValues = false )
    {
        ...
        {
            $language = new LOCALIZATION_DTO_Language();
            $language->
    ...
    /**
     * @param LOCALIZATION_DTO_Language $dto
     */
    public function save(LOCALIZATION_DTO_Language $dto) {
        $language = $this->languageDao->save($dto);
        $this->languageDao->clearCache();
        return $language;
    }
    ...
    public function findByIsoCode($isoCode) {
        return $this->languageDao->findByIsoCode($isoCode);
    }

    /**
     * @param LOCALIZATION_DTO_Language $language
     */
    public function delete(LOCALIZATION_DTO_Language $language) {
    ...
    public function importPrefixFromDir( $path, $refreshCache = true, $addLanguage = false, $updateValues = false )
    {
        ...
        foreach ( $langsToImport as $langToImport )
        {
            if ( !$this->findByTag($langToImport['tag']) )
            {
                ...
                $dto = new LOCALIZATION_DTO_Language();
    ...
    public function getCurrent() {
        if($this->currentLanguage === null) {
            if(!empty(OW::getSession()->get(self::LANG_ID_VAR_NAME))) {
                $languageId = (int)OW::getSession()->get(self::LANG_ID_VAR_NAME);
                $language = $this->findById($languageId);
            } elseif(OW::getRequest()->readCookie(self::LANG_ID_VAR_NAME)) {
                $languageId = (int)OW::getRequest()->readCookie(self::LANG_ID_VAR_NAME);
                $language = $this->findById($languageId);
            } elseif((int)OW::getConfig()->getValue(LOCALIZATION_BOL_Service::KEY, 'detectLanguage')) {
                $isoCode = substr($_SERVER["HTTP_ACCEPT_LANGUAGE"], 0, 2);
                $language = $this->findByIsoCode($isoCode);
            }
            if(!$language) {
                $language = $this->languageDao->getCurrent();
            }
     ...
     */
    public function setCurrentLanguage(LOCALIZATION_DTO_Language $language, $loadFromCache = true) {
        OW::getSession()->set(self::LANG_ID_VAR_NAME, $language->getId());
        OW::getRequest()->setCookie(self::LANG_ID_VAR_NAME, strval($language->getId()));
        $this->currentLanguage = $language;
        if($loadFromCache) {
            $this->loadFromCache();
        }
